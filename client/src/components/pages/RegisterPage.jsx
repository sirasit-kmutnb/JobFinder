...
const RegisterPage = () => {
  ...
  const {email, username, password, confirmpassword, role, info1, info2} = auth
  ...
  const submitForm =(e)=>{
    e.preventDefault()
    axios
    .post(`${import.meta.env.VITE_APP_API}/register`, {email, username, password, confirmpassword, role, info1, info2})
    .then((response)=>{
      alert("Successful")
    ...
    <AnimatedPage>
      ...
      <form onSubmit={submitForm}>
        ...
        <div className="form-group mt-3">
          <label>email</label>
          <input type="text" className="form-control" value={email} onChange={inputValue("email")} />
        </div>
        ...
        <div className="form-group mt-3">
          ...
          <input type="password" className="form-control" value={confirmpassword} onChange={inputValue("confirmpassword")} />
        </div>

        <div className="form-group mt-3">
          <label>Role</label>
          <select className="form-select" aria-label="Default select example" value={role} onChange={inputValue("role")}>
